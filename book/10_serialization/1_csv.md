...
В это примере мы сперва записываем строки из списка в файл, а затем отображаем содержимое файла.
...
Конечно, в этом случае, у нас достаточно простой пример, но когда в строках больше значений, то кавычки позволяют указать где начинается и заканчивается значение.

Модуль позволяет управлять этим.
Для того, чтобы все строки записывались в файл csv с кавычками, надо изменить наш скрипт так (файл csv_write_ver2.py):
```python
import csv
