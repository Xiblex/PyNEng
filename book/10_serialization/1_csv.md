...
Но, было бы лучше, чтобы все строки были в кавычках. Модуль позволяет управлять этим. Для того, чтобы все строки записывались в файл csv с кавычками, надо изменить наш скрипт так (файл csv_write_ver2.py):
```python
import csv
