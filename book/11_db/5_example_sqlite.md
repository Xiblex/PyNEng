...
Комментарии к скрипту:
* считываем параметры key, value из аргументов, которые передали скрипту
 * удаляем из списка keys, выбранный ключ. Таким образом мы получаем список параметров, которые нам нужно вывести
* подключаемся к БД
 ...
 * ```cursor.execute("select * from dhcp where %s = ?" % key, (value,))```
  * в SQL  мы можем подставлять значения, через знак вопроса, но не можем подставлять имя столбца. Поэтому, имя столбца мы подставляем через форматирование строк, а значение - штатным средством SQL.
    * Обратите внимание на ```(value,)``` таким образом мы передаем кортеж с одним элементом
...
* не проверяется количество аргументов, которые мы передаем скрипту
 * если передадим 0 или 1 или 3 аргумента, возникнет ошибка
 * не проверяется правильность аргументов
* так как информацию мы можем собирать с разных коммутаторов, то надо добавить поле, которое указывает на каком коммутаторе была найдена запись
