...
По умолчанию, в конце каждого выражения, переданного в print(), будет перевод строки. Если необходимо, чтобы после вывода выражения не было перевода строки, надо в качестве параметра функции print указать дополнительный аргумент end. Если его не указывать, от он будет равен \n, поэтому к строкам в print и добавляется перевод строки.
