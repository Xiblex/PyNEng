...
####```remove()```
Метод __remove()__ удаляет указанный элемент. Но в нем надо указывать сам элемент, который надо удалить, а не его номер в списке. Кроме того, remove() не возвращает удаленный элемент: 
```python
